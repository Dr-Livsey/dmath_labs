...
Graph::Graph(size_t initSize)
{
	...
	resizeMatrix(WeighedMatrix, initSize);
}
...
Matrix & resizeMatrix(Matrix &m, size_t s)
{
...
void Graph::showMatrix(const Matrix&m, std::string name) const
{
	...
	if (name != "Incidence Matrix")
	{
		...
		for (auto i = m.cbegin(); i != m.cend(); i++)
			cout << std::setw(4) << "[" + std::to_string(i - m.cbegin() + 1) + "]";
		cout << endl;
	}
	...
	for (auto row = m.cbegin(); row != m.cend(); row++)
	{
		...
		cout << std::setw(4) << "[" + std::to_string(row - m.cbegin() + 1) + "]";
...
/*объединение компонент связности в один граф*/
Matrix operator||(const Matrix & a, const Matrix & b)
{
	size_t resultSize = std::max(a.size(), b.size());

	Matrix m1, m2;
	m1 = a;
	m2 = b;

	resizeMatrix(m1, resultSize);
	resizeMatrix(m2, resultSize);
	...
	auto readAttribute = [](std::ifstream &initfile) -> std::vector<std::string>
	{
			...
			boost::regex_match(attr, matches, w_attr) == false)
		{
			...
			throw std::exception("Wrong attribute. Please, specify the init form of the graph.\n"
								...
								"                    AL -> Adjacency List\n"
								"                    IM -> Icidence Matrix\n"
								"                    WM -> Weighted Matrix(Ford-Bellman, Floyd)\n"
								"                    w  -> Weighted Matrix\n");
		}
		else if (matches[1].str() == "WM")
		{
...
void Graph::readWMatrix(std::ifstream &initfile)
{
	std::list<std::string> lRows = getRowList(initfile);
	...
	{
		AdjacencyMatrix.resize(Rows + 1);
		WeighedMatrix.resize(Rows + 1);

		std::string::const_iterator i(row.cbegin());
		boost::smatch matches;
		while (boost::regex_search(i, row.cend(), matches, boost::regex("(-?\\d{1,}|i)(?: *|$|\\n)")))
		{
			i += matches.position() + matches[0].length();
			short newVertexValue;
			short weight;
			...
			{
				/*вершины смежны*/
				weight = boost::lexical_cast<short>(matches[1]);
				newVertexValue = (weight == 0) ? 0 : 1;
			}

			AdjacencyMatrix[Rows].push_back(newVertexValue);
			WeighedMatrix[Rows].push_back(weight);
		}
		Rows++;
	}
	...
	if (AdjacencyMatrix.size() != AdjacencyMatrix[0].size())
	{
		throw std::exception("Weighted Matrix is incrorrect.");
	}
	else
	{
		for (int i = 0; i < WeighedMatrix.size(); i++)
	...
	isWeighted = true, isOriented = true;
...
void Graph::showPrima()
{
	...
	for (auto c : components)
	{
		result = result || algorithmPrima(c);
	}
	...
	for (int i = 0; i < result.size(); i++)
	{
		...
		{
			if (AdjacencyMatrix[i][j]) sum += result[i][j];
		}
	}
...
Matrix Graph::algorithmPrima(short v)
{
	if (AdjacencyMatrix.empty())
		throw std::exception("Graph is empty.");
...
{
	Matrix w_matrix;
	resizeMatrix(w_matrix, dist.size());
