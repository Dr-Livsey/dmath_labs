...
void Interface::get_function(Graph *graph, int cmd_num, const std::deque<std::string>&args)
{
	...
	case 35:/*вывод матрицы весов*/
		...
		break;
	case 36: /*алгоритм Флойда*/
		if (args.size() != 2)
			throw std::exception("Invalid amount of arguments in \"floyd\"");
		{
			short a, b;
			try
			{
			...
			{
				throw std::exception("Invalid arguments in \"floyd\"");
			}
			graph->floyd(a, b);
		}
		break;
	...
	case 37: /*алгоритм Форда - Беллмана*/
			...
			throw std::exception("Invalid amount of arguments in \"ford-bellman\"");
		{
			...
			{
				throw std::exception("Invalid arguments in \"bellman\"");
			}
